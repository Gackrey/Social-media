...
import { getID, updateUser, updateAccountWithPassword, updateAccountWithoutPassword } from "../../features/auth/authSlice";
...
export const UpdateDetails = () => {
        ...
        if (changePassword && phone) {
            ...
            await dispatch
                (updateAccountWithPassword(
                    { firstname: firstName, lastname: lastName, email, phone, oldpassword, newpassword, token }
                ))
            ...
            if (phone) {
                ...
                await dispatch(
                    updateAccountWithoutPassword(
                        { firstname: firstName, lastname: lastName, email, phone, token }
                    )
                )
                successToast("Updated account details")
                navigate("/")
    ...
    return (
        <div className="update-details">
            <ToastContainer />
            <div className="tabs">
                <span className={tab === 1 ? "tab active" : "tab"}
                    onClick={() => setTabs(1)}
                >Update Account</span>
                <span className={tab === 2 ? "tab active" : "tab"}
                    onClick={() => setTabs(2)}
                >Update User</span>
            </div>
            ...
            >
                ...
                <input type="text" name="firstname" className="input" placeholder="First Name" value={firstName} required
                    onChange={(e) => setFirstName(e.target.value)}
                />
                <label htmlFor="lastname" >Last Name</label>
                <input type="text" name="lastname" className="input" placeholder="Last Name" value={lastName} required
                    onChange={(e) => setlastName(e.target.value)}
                />
                <label htmlFor="email" >Email</label>
                <input type="email" name="email" className="input" placeholder="Email" value={email} required
                    onChange={(e) => setEmail(e.target.value)}
                />
                <label htmlFor="phone" >Phone No.</label>
                <input type="number" name="phone" className="input" placeholder="Phone Number" value={phone} required
                    onChange={(e) => {
            ...
            >
                ...
                <textarea className="input input-bio" name="bio" placeholder="Add bio" value={bio} required
                    draggable={false}

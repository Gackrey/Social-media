...
export const SignInUser = createAsyncThunk(
  ...
  async ({ firstName, lastName, email, phone, password }: SignInType) => {
    const response = await axios.post<signinData>(`${API_URL}/account/signup`, {
      firstname: firstName,
      lastname: lastName,
      email,
      phone,
...
export const getUserData = createAsyncThunk(
  ...
  async ({ _id }: getUserType) => {
    const response = await axios.post<authStateType>(
...
export const LoginUser = createAsyncThunk(
  ...
  async ({ email, password }: LoginType) => {
    const response = await axios.post<loginData>(`${API_URL}/account/login`, {
      email,
      password,
    });
    return response.data;
...
export const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        updateAccountWithoutPassword.pending ||
          updateAccountWithPassword.pending,
        (state) => {
          state.status = "loading";
        }
      )
      .addCase(
        updateAccountWithoutPassword.fulfilled ||
          updateAccountWithPassword.fulfilled,
        (state, action) => {
          state.status = "done";
          state.isUserLogin = true;
          state.token = action.payload.id;
      ...
      .addCase(
        updateAccountWithoutPassword.rejected ||
          updateAccountWithPassword.rejected,
        (state, action) => {
          state.status = "error";
        }
      )
      ...
      .addCase(
        followUser.fulfilled || unFollowUser.fulfilled,
        (state, action) => {
          state.status = "done";
...
export const getID = (state: authReducerType) => state.auth.id;
export const getStatus = (state: authReducerType) => state.auth.status;
export const getUserFollowingList = (state: authReducerType) =>
  state.auth.following;
export const getUserFollowerList = (state: authReducerType) =>
